...
test.describe('API Challenges', () => {
  test.beforeAll('Get X-Challenger token - POST token (201)', async ({ request }) => {
  ...
  test('@PUT /challenger/guid CREATE', async ({ request }) => {
    ...
    const xAuthToken = body['xAuthToken'];
  ...
  test('@POST /todos (201) all', async ({ request }) => {
    ...
    expect(errorResponse).toHaveProperty('errorMessages', ['ERROR: Cannot add instance, maximum limit of 20 reached']);
    ...
    const body = await getFullListTodos.json()
    // console.log(body)
